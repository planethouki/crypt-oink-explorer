<template>
  <section>
    <div class="d-flex d-md-none align-items-end justify-content-between">
      ...
      <b-nav tabs="tabs">
        <template v-for="tab in tabs">
          <b-nav-item v-if="type !== tab.id" :to="{ name: 'shop-page-type', params: { type: tab.id, page } }" active="active">
    ...
    <div class="d-none d-md-flex align-items-end justify-content-between">
      ...
      <b-nav tabs="tabs">
        <template v-for="tab in tabs">
          <b-nav-item :to="{ name: 'shop-page-type', params: { type: tab.id, page } }" :active="type === tab.id">
            <font-awesome-icon :icon="['fas', tab.icon]"></font-awesome-icon>
          </b-nav-item>
        </template>
      </b-nav>
    ...
    <section id="tons">
      <template v-for="tab in tabs">
        <component :is="`Auction${tab.text}`" v-if="type === tab.id" name="sell" :currentTons="currentTons" :asyncTonsCache="asyncTonsCache"></component>
      </template>
    </section>
  </section>
</template>
