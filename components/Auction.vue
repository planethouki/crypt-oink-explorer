  ...
  methods: {
    ...
    updatePartTons() {
      this.partTons = this.tokensId.map(tokenId => ({
        ...
        getAuction: this.$contracts[this.contract].methods
          .getAuction(tokenId)
          .call(),
        getCurrentPrice: this.$contracts[this.contract].methods
    ...
    updateTons() {
      ...
      this.partTons.map((ton, index) => {
        const promiseCp = ton.getCurrentPrice
          .then(price => {
        ...
        const promiseAc = ton.getAuction
          .then(entity => {
            if (ton.id !== this.tons[index].id) return
            const tons = this.tons.slice()
